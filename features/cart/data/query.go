...
func (repo *cartData) SelectByToken(token int) ([]cart.Core, error) {
	...
	tx := repo.db.Model(&Product{}).Select(" carts.id, carts.quantity, products.name, products.images, products.price, carts.user_id, carts.products_id").Joins("inner join carts on carts.product_id = products.id").Where("carts.user_id = ?", token).Scan(&dataCart)
	if tx.Error != nil {
		return nil, tx.Error
	}
